    ...
    <div class="mx-auto max-w-screen-xl">
        ...
        <div class="bg-white relative border-solid border border-gray-300 rounded-lg sm:rounded-lg overflow-hidden">
            ...
            <div class="overflow-x-auto">
                        ...
                        @forelse ($sucursales as $sucursal)
                            <tr class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">
                                    #{{ $sucursal->id }}</th>
                                <td class="px-4 py-3">{{ $sucursal->zona }}</td>
                                <td class="px-4 py-3">{{ $sucursal->numero }}</td>
                                <td class="px-4 py-3">{{ $sucursal->sucursal }}</td>
                                <td class="px-4 py-3">{{ $sucursal->direccion }}</td>
                                <td class="px-4 py-3">{{ $sucursal->created_at }}</td>
                                <td class="px-4 py-3 flex items-center justify-center align-items-center">
                                    ...
                                    <form action="{{ route('sucursales.destroy', $sucursal->id)}}" method="post" onclick="Eliminar(this, '{{ $sucursal->sucursal }}')">
                                        @csrf
                                        @method('delete')
                ...
                @empty
                    <p class="text-center text-slate-950 my-5">Todavia no existen sucursales registrados</p>
                    @endforelse
                </table>
            </div>
            @if (count($sucursales))
...
<script>
    function Eliminar(e, sucursal) {
        Swal.fire({
            title: "Estas seguro?",
            text: `Estas a punto de eliminar al sucursal "${sucursal}"`,
            icon: "warning",
